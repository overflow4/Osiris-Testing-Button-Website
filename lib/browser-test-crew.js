...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    } else if (looksLikeRegisteredPhone) {
      // The caller passed a phone directly. Look up the matching user_id via
      // Supabase so the seed can attach a job to that specific tech.
      console.log(`[browser-crew][${requestId}] Caller-provided phone ${credentials.email} — looking up user_id via Supabase...`);
      const fromDb = await discoverCrewViaSupabase().catch(() => null);
      if (fromDb) {
        // Prefer the row whose phone matches the caller-provided one
        const callerDigits = credentials.email.replace(/[^0-9]/g, '').slice(-10);
        const match = fromDb.phone && fromDb.phone.replace(/[^0-9]/g, '').slice(-10) === callerDigits ? fromDb : null;
    ...
    // Pass technicianPhone alongside userId so the seeder can fall back to a
    // Supabase phone→user_id lookup if the API didn't expose user_id.
    let seedDetail;
    if (!discoveredTech) {
      seedDetail = `skipped: no technician discovered (no /api/teams response)`;
    } else if (!discoveredTech.userId && !discoveredTech.phone) {
      seedDetail = `skipped: discovered tech has no userId or phone (rawKeys=${JSON.stringify(discoveredTech.rawKeys || [])})`;
    } else {
      try {
        ...
        seedDetail = `failed: ${e.message.slice(0, 300)}`;
