const { createSession, osirisLogin, getLoginDiagnostics, discoverCrewPhone } = require('./browser-helpers');
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // Step 1: Resolve crew credentials.
    // To exercise the job-detail flow we MUST log in as an actual registered
    // technician (otherwise the schedule is empty and 7 downstream checks
    // false-fail). Two-tier discovery:
    //   1. /api/teams via admin login (gets phone + user_id + business_id)
    //   2. Supabase service-role fallback if (1) fails
    let crewCreds = credentials;
    let discoveredTech = null;
    let dbDiscoveryDiag = null;
    const looksLikeRegisteredPhone = credentials?.email && /^\+?\d{10,}/.test(credentials.email.replace(/[^0-9+]/g, ''));
    if (!looksLikeRegisteredPhone) {
      console.log(`[browser-crew][${requestId}] No crew phone provided — discovering one via admin API...`);
      discoveredTech = await discoverCrewPhone(page, baseUrl, null);
      if (!discoveredTech || !discoveredTech.userId) {
        console.log(`[browser-crew][${requestId}] API discovery returned ${discoveredTech ? 'no user_id' : 'null'} — trying Supabase-direct fallback...`);
        const fromDb = await discoverCrewViaSupabase().catch(e => {
          console.log(`[browser-crew][${requestId}] Supabase discovery threw: ${e.message}`);
          return { _diag: `threw: ${e.message}` };
        });
        dbDiscoveryDiag = fromDb?._diag || null;
        if (fromDb?.phone) discoveredTech = fromDb;
      }
      if (discoveredTech?.phone) {
        crewCreds = { email: discoveredTech.phone, password: '' };
        console.log(`[browser-crew][${requestId}] Using discovered crew member: ${discoveredTech.name} (${discoveredTech.phone}, user=${discoveredTech.userId || 'unknown'})`);
      } else {
        console.log(`[browser-crew][${requestId}] No registered technician found — login will likely land on empty schedule`);
      }
    } else {
      console.log(`[browser-crew][${requestId}] Caller-provided phone ${credentials.email} — looking up matching tech via Supabase...`);
      const fromDb = await discoverCrewViaSupabase().catch(() => null);
      dbDiscoveryDiag = fromDb?._diag || null;
      if (fromDb?.phone) {
        const callerDigits = credentials.email.replace(/[^0-9]/g, '').slice(-10);
        const match = fromDb.phone.replace(/[^0-9]/g, '').slice(-10) === callerDigits ? fromDb : null;
        discoveredTech = match || fromDb;
      } else {
        // Even without DB discovery, try seeding using the caller's phone.
        // seedCrewJob.lookupTechUserIdByPhone walks more candidates and may
        // resolve user_id even when the broad-discovery probe didn't.
        discoveredTech = { phone: credentials.email, name: 'caller-provided' };
      }
    }

    // Step 1b: Seed a job for today so the crew has something to click into.
    // Uses Supabase service-role to insert customer + job + cleaner_assignment
    // with phone_number=TEST_PHONE so existing resetTestData cleans them up.
    let seedDetail;
    const phoneForSeed = discoveredTech?.phone || (looksLikeRegisteredPhone ? credentials.email : null);
    if (!phoneForSeed && !discoveredTech?.userId) {
      seedDetail = `skipped: no phone or user_id available. DB discovery diag: ${dbDiscoveryDiag || 'not attempted'}`;
    } else {
      try {
        const seeded = await seedCrewJob({
          technicianUserId: discoveredTech?.userId,
          technicianBusinessId: discoveredTech?.businessId,
          technicianPhone: phoneForSeed,
        });
        seedDetail = `job=${seeded.jobId} assignment=${seeded.assignmentId}`;
        console.log(`[browser-crew][${requestId}] Seeded crew job: ${seedDetail}`);
      } catch (e) {
        seedDetail = `failed: ${e.message.slice(0, 400)}. DB discovery: ${dbDiscoveryDiag || 'n/a'}`;
        console.log(`[browser-crew][${requestId}] Seed failed: ${e.message}`);
      }
    }
    results.push({
      section: 'seed_job',
      label: 'Seed crew job',
      passed: typeof seedDetail === 'string' && seedDetail.startsWith('job='),
      detail: seedDetail,
      screenshot: null,
    });

    // Step 2: Login as technician via cleanmachine.live role-selection screen.
    ...
    const portalInfo = await page.evaluate(() => {
      ...
      const noJobsToday = /no jobs scheduled|enjoy your day off|0 jobs/i.test(body);
      const buttons = [...document.querySelectorAll('button')].filter(b => b.offsetParent !== null);
      const jobCards = document.querySelectorAll('[class*="job"], [class*="card"], [class*="appointment"]');

      // Look for crew portal specific elements. Note "Log out" has a space —
      // /logout/ doesn't match; need /log\s*out/.
      ...
      const hasLogout = buttons.some(b => /log\s*out|sign\s*out/i.test(b.textContent));
      // Date nav can be text ("Prev"/"Next"/"Today") OR icon-only chevron
      // buttons (aria-label, SVG-only, or class hint). Accept any of those.
      const hasNavigation = buttons.some(b => {
        const text = (b.textContent || '').trim();
        if (/prev|next|today|go today/i.test(text)) return true;
        const aria = (b.getAttribute('aria-label') || '').toLowerCase();
        if (/prev|next|today|previous/.test(aria)) return true;
        const cls = (b.className || '').toString().toLowerCase();
        if (/prev|next|chevron|arrow/.test(cls)) return true;
        // SVG-only buttons (no text content) often used for date stepping —
        // count them if they sit next to a date string in the page text.
        if (!text && b.querySelector('svg, [class*="icon"]')) return true;
        return false;
      });

      return {
        isError, hasSchedule, hasViewToggle, hasAvailability, hasLogout, hasNavigation, noJobsToday,
