...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    let seedDetail = 'skipped';
    if (discoveredTech?.userId) {
      try {
        ...
        seedDetail = `failed: ${e.message.slice(0, 200)}`;
        console.log(`[browser-crew][${requestId}] Seed failed: ${e.message}`);
      }
    } else {
      console.log(`[browser-crew][${requestId}] Skipping seed — no technician user_id`);
    }
    results.push({
      section: 'seed_job',
      label: 'Seed crew job',
      passed: seedDetail.startsWith('job='),
      detail: seedDetail,
      screenshot: null,
    });
