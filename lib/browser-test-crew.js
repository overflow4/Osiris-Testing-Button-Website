const { createSession, osirisLogin } = require('./browser-helpers');
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!loggedIn) {
      const failShot = await page.screenshot({ fullPage: false, type: 'jpeg', quality: 50 }).catch(() => null);
      ...
      return res.status(200).json({
        ...
        results: [{
          ...
          passed: false,
          detail: `Login failed — could not reach crew portal at ${page.url?.() || baseUrl}`,
          screenshot: failShot ? failShot.toString('base64') : null,
        }],
        businessName,
      });
    }
