...
const { seedCrewJob } = require('./seed-crew-job');
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // If the caller didn't pass a registered crew phone, log in as admin first
    // and pull a real one from /api/teams (preferring TEST accounts). Then log
    // out before re-logging-in as technician.
    ...
    if (!looksLikeRegisteredPhone) {
      ...
      if (discoveredTech) {
        ...
        console.log(`[browser-crew][${requestId}] Using discovered crew member: ${discoveredTech.name} (${discoveredTech.phone})`);
      } else {
        console.log(`[browser-crew][${requestId}] Crew discovery failed — falling back to provided credentials`);
      }
    }
