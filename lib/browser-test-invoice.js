...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const agreementInfo = await page.evaluate(() => {
      // Handle native checkboxes, custom toggles, AND text buttons like "Tap to Accept Service Agreement"
      ...
      const isAgreement = (el) => {
        const context = (el.closest('label')?.textContent || el.parentElement?.textContent || el.getAttribute('aria-label') || el.textContent || '').toLowerCase();
        return /agree|terms|service agreement|conditions|policy|accept|acknowledge/i.test(context);
      };
      ...
      // Buttons with agreement text (e.g. "Tap to Accept Service Agreement", "I agree to terms")
      // Skip the main checkout button — agreement buttons usually don't say "book/pay/checkout".
      for (const btn of allButtons) {
        if (btn.disabled) continue;
        if (btn.offsetParent === null) continue;
        const txt = (btn.textContent || '').trim().toLowerCase();
        if (!txt || txt.length > 200) continue;
        if (!/tap to accept|accept.*agreement|i agree|click to accept|accept terms|acknowledge/i.test(txt)) continue;
        if (/book|pay now|checkout|save.*card/i.test(txt)) continue;
        btn.click();
        clickedLabels.push((btn.textContent || '').trim().slice(0, 40));
        checkedCount++;
      }
