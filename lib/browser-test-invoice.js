...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const agreementInfo = await page.evaluate(() => {
      ...
      const customToggles = [...document.querySelectorAll('[role="switch"], [role="checkbox"], button[aria-checked="false"], [aria-checked="false"]')];
      const allButtons = [...document.querySelectorAll('button, [role="button"]')];
      const clickableEls = [...document.querySelectorAll('div[onclick], span[onclick], a[href="#"], a[role="button"]')];
      ...
      const isAgreement = (el) =>
        /agree|terms|service agreement|conditions|policy|accept|acknowledge|consent/i.test(ctxOf(el));
      // Patterns to recognize agreement-acting buttons by their own text.
      const agreementBtnRe = /tap to accept|accept.*agreement|accept.*terms|i agree|click to accept|acknowledge|i consent|read.*agreed|accept service|accept.*policy/i;
      const checkoutBtnRe = /book|pay now|checkout|save.*card|complete.*booking|confirm.*book|proceed to payment/i;
      ...
      for (const cb of nativeCheckboxes) {
        ...
        if (!isAgreement(cb) && nativeCheckboxes.length > 1) continue;
        const label = cb.closest('label');
        if (label) { label.click(); } else { cb.click(); }
        clickedLabels.push((cb.closest('label')?.textContent || '').trim().slice(0, 40) || '(checkbox)');
        checkedCount++;
      }
      ...
      for (const t of customToggles) {
        ...
        t.click();
        clickedLabels.push((t.closest('label')?.textContent || t.textContent || '').trim().slice(0, 40) || '(toggle)');
        checkedCount++;
      }

      for (const btn of [...allButtons, ...clickableEls]) {
        if (btn.disabled) continue;
        if (!visible(btn)) continue;
        const txt = (btn.textContent || '').trim();
        const lower = txt.toLowerCase();
        if (!lower || lower.length > 200) continue;
        if (!agreementBtnRe.test(lower)) continue;
        if (checkoutBtnRe.test(lower)) continue;
        btn.click();
        clickedLabels.push(txt.slice(0, 40));
        checkedCount++;
      }
      ...
      // If we didn't find anything, capture what's actually on the page so
      // the test report lets us see what to match against next time. The
      // page can change UI shape on the server side without us knowing.
      let diag = null;
      if (checkedCount === 0) {
        const inv = (s) => (s || '').toString().trim().replace(/\s+/g, ' ').slice(0, 80);
        const visibleButtons = [];
        for (const b of allButtons) {
          if (!visible(b)) continue;
          const t = inv(b.textContent);
          if (!t) continue;
          visibleButtons.push({
            text: t,
            disabled: !!b.disabled,
            ariaChecked: b.getAttribute('aria-checked') || null,
            type: b.tagName === 'BUTTON' ? b.type : null,
          });
          if (visibleButtons.length >= 25) break;
        }
        const agreementHits = [];
        const all = [...document.querySelectorAll('div, span, p, label, section, h1, h2, h3, h4')];
        const seen = new Set();
        for (const el of all) {
          if (!visible(el)) continue;
          const t = (el.innerText || '').trim();
          if (!t || t.length > 240) continue;
          if (!/agree|terms.*conditions|service agreement|tap to accept|click to accept|i acknowledge|consent|accept.*policy|read.*agreed/i.test(t)) continue;
          const key = t.slice(0, 100);
          if (seen.has(key)) continue;
          seen.add(key);
          agreementHits.push({ tag: el.tagName, text: inv(t).slice(0, 200) });
          if (agreementHits.length >= 8) break;
        }
        diag = {
          checkboxes: nativeCheckboxes.length,
      ...
      return { checkedCount, clickedLabels, diag };
    });
    ...
    if (checkedAgreement) {
      ...
      console.log(`[browser-invoice][${requestId}] No agreement checkboxes found — diag: ${JSON.stringify(agreementInfo.diag).slice(0, 500)}`);
    }
    let agreementDetail;
    if (checkedAgreement) {
      agreementDetail = `Checked ${agreementInfo.checkedCount} box(es): ${agreementInfo.clickedLabels.join(' | ')}`;
    } else {
      const d = agreementInfo.diag || {};
      const btnPreview = (d.visibleButtons || []).slice(0, 8).map(b => `"${b.text}"${b.disabled?'(d)':''}`).join(', ');
      const hitPreview = (d.agreementHits || []).slice(0, 3).map(h => `${h.tag}:"${h.text}"`).join(' | ');
      agreementDetail = `No agreement matched. checkboxes=${d.checkboxes||0}, toggles=${d.customToggles||0}.`;
      if (btnPreview) agreementDetail += ` Visible buttons: ${btnPreview}.`;
      if (hitPreview) agreementDetail += ` Agreement-text on page: ${hitPreview}.`;
    }
    checks.push({ t: 'Agreement checkbox accepted', s: checkedAgreement, d: agreementDetail });
