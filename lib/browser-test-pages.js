...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // Step 1: Login if needed (Osiris flow: role button → credentials → INITIALIZE)
    if (loginFirst && credentials?.email) {
      ...
      try {
        // Phase A: Click role button FIRST (Osiris shows role selection before credentials)
        const roleBtn = await page.$('button:has-text("Operator"), button:has-text("Owner"), button:has-text("Admin"), button:has-text("Staff")');
        if (roleBtn) {
          console.log(`[browser-pages][${requestId}] Clicking role button...`);
          await roleBtn.click();
          await page.waitForTimeout(2500);
        }

        // Phase B: Fill credentials (only if password field present)
        ...
        if (hasPasswordField) {
          ...
          // Phase C: Click INITIALIZE / submit
      ...
      const loggedIn = !stillOnLogin || endUrl !== startUrl;
      const loginScreenshot = await page.screenshot({ fullPage: false, type: 'jpeg', quality: 50 });
      results.push({
        section: 'login',
        label: 'Login',
        passed: loggedIn,
        detail: loggedIn ? `Logged in → ${endUrl.slice(0, 80)}` : `Stuck on login page`,
