...
// Make the email unique per run (gmail "+tag" routes to the same inbox but
// looks unique to the server). Without this, the same name/phone/email
// hammering the lead webhook test-after-test triggers backend dedup or
// rate-limit logic, and the bot silently drops the lead → no SMS, even
// though the webhook returns 200 OK with a new lead id.
function buildTestData() {
  const stamp = Date.now().toString(36);
  return {
    name: 'Alex Thompson',
    phone: '4246771145',
    email: `alex.thompson.test+${stamp}@gmail.com`,
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    log('test data', true, `email=${testData.email}`);

    // Capture the actual webhook POST + its response so we can report what
    // was sent AND whether the server actually accepted it. Seeing the
    // request alone doesn't prove anything — a 4xx, dedupe-200, or silent
    // success all look the same on the way out.
    ...
    page.on('response', async r => {
      const u = r.url();
      if (r.request().method() === 'POST' && /\/api\/(webhooks?|leads?|book|submit|inquir|forms?)|webhook/i.test(u)) {
        if (webhookCall && !webhookCall.responseStatus) {
          webhookCall.responseStatus = r.status();
          try { webhookCall.responseBody = (await r.text()).slice(0, 500); } catch {}
        }
      }
    });
    ...
    if (bizType === 'house_cleaning') {
      ...
      }, { bed: testData.bedrooms, bath: testData.bathrooms });
    ...
    // Wait for the network call AND its response to land — the response
    // status is the actual signal of whether the lead was accepted.
    const deadline = Date.now() + 12000;
    while ((!webhookCall || webhookCall.responseStatus == null) && Date.now() < deadline) {
