 ...
 * Returns { phone, name, userId, businessId } or null if no candidates.
 *
 * This is the fallback for when discoverCrewPhone via /api/teams fails
 * (admin creds wrong, endpoint missing, RLS blocking, etc). Service-role
 * bypasses RLS so we always get a result if there's any tech in the DB.
 */
async function discoverCrewViaSupabase() {
  if (!SUPABASE_KEY) return null;
  // Tables that might hold the technician roster — try each. The first
  // one that has rows wins.
  const candidates = [
    { table: 'team_members', phoneCol: 'phone', idCol: 'user_id', filter: 'is_active=eq.true&employee_type=eq.technician' },
    { table: 'team_members', phoneCol: 'phone_number', idCol: 'user_id', filter: 'is_active=eq.true&employee_type=eq.technician' },
    { table: 'team_members', phoneCol: 'phone', idCol: 'user_id', filter: 'is_active=eq.true' },
    { table: 'cleaners', phoneCol: 'phone', idCol: 'user_id', filter: 'is_active=eq.true' },
    { table: 'cleaners', phoneCol: 'phone', idCol: 'id', filter: 'is_active=eq.true' },
    { table: 'technicians', phoneCol: 'phone', idCol: 'user_id', filter: '' },
    { table: 'profiles', phoneCol: 'phone', idCol: 'user_id', filter: 'role=eq.technician' },
    { table: 'profiles', phoneCol: 'phone', idCol: 'id', filter: 'role=eq.technician' },
  ];

  for (const { table, phoneCol, idCol, filter } of candidates) {
    try {
      const select = `select=*,${phoneCol},${idCol}`;
      ...
      // Prefer test accounts so we don't disturb real techs
      const test = usable.find(r => /test/i.test(`${r.name || ''} ${r.username || ''} ${r.first_name || ''} ${r.last_name || ''}`));
      ...
      return {
        ...
        name: picked.name || picked.username || picked.first_name || 'unknown',
        userId: picked[idCol],
        businessId: picked.business_id || picked.tenant_id || null,
      };
    } catch (e) {
      // Table or column missing — keep walking
    }
  }
  return null;
}
