 ...
 *
 * If any step fails, throws an Error with detail. Caller should catch and
 * surface in the test report rather than aborting the crew test entirely
 * (we can still test login + portal load even without a job to click).
 */
async function seedCrewJob({ technicianUserId, technicianBusinessId }) {
  assertConfigured();
  if (!technicianUserId) throw new Error('seedCrewJob: technicianUserId is required');
