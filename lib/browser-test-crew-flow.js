const { createSession, osirisLogin, getLoginDiagnostics, shot, safeClick, closeModal, runCheck } = require('./browser-helpers');
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // osirisLogin clicks the Employee/Technician role button (NOT
    // Operator/Owner) and handles logging out of any active owner session
    // first via the bottom-left business-slug menu.
    await runCheck(results, 'Crew Portal Load', async () => {
      ...
      if (!loggedIn) {
        ...
        return {
          passed: false,
          detail: `Login failed @ ${diag.phase || 'unknown'}: ${diag.reason || 'no diagnostic'}. Visible buttons: [${clickablesPreview}]`,
      ...
      return {
        passed: !info.isError && (info.buttons > 2 || info.hasSchedule),
