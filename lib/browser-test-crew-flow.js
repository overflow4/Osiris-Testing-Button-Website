...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // Auto-discover a real crew phone if none was provided. This logs in as
    // admin first, hits /api/teams to fetch the technician roster, picks a
    // TEST account if available, then drives logout + technician login.
    ...
    if (!looksLikeRegisteredPhone) {
      ...
      if (discoveredTech) crewCreds = { email: discoveredTech.phone, password: '' };
    }
    ...
    // cascades into 7 false-FAIL downstream checks.
