...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    await runCheck(results, 'Crew Portal Load', async () => {
      ...
      return {
        passed: loggedIn && !info.isError && (info.buttons > 2 || info.hasSchedule),
        detail: !loggedIn ? 'Technician login failed' : info.isError ? 'Portal error after login' : `${info.buttons} buttons, schedule:${info.hasSchedule}`,
