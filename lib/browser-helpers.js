...
async function discoverCrewPhone(page, baseUrl, adminCreds) {
  ...
  try {
    ...
    // The API response shape varies — the user-id field has been observed as
    // user_id, id, member_id, cleaner_id, technician_id, account_id, uid, or
    // even nested under user.id. We try every variant and walk into nested
    // objects as a fallback.
    const extractUserId = (m) => {
      const directKeys = [
        'user_id', 'userId', 'id', 'member_id', 'memberId',
        'cleaner_id', 'cleanerId', 'technician_id', 'technicianId',
        'account_id', 'accountId', 'employee_id', 'employeeId',
        'uid', 'auth_id', 'authId', 'profile_id', 'profileId',
      ];
      for (const k of directKeys) if (m[k]) return m[k];
      // Walk one level into common nested containers
      for (const container of ['user', 'profile', 'account', 'auth', 'cleaner', 'technician']) {
        const inner = m[container];
        if (inner && typeof inner === 'object') {
          for (const k of directKeys) if (inner[k]) return inner[k];
        }
      }
      return null;
    };
    ...
    console.log(`[discoverCrewPhone] picked ${picked.name} (${picked.phone}, user=${picked.userId || 'NOT FOUND'}, biz=${picked.businessId}) — ${testMember ? 'TEST account' : 'first available'}`);
    if (!picked.userId) {
      console.log(`[discoverCrewPhone] member keys were: ${JSON.stringify(picked.rawKeys)}`);
      if (firstRawSnapshot) console.log(`[discoverCrewPhone] sample member: ${JSON.stringify(firstRawSnapshot.sample).slice(0, 500)}`);
    }
