...
async function discoverCrewPhone(page, baseUrl, adminCreds) {
  ...
  try {
    ...
    for (const team of result.data || []) {
      for (const m of team.members || []) {
        if (m.phone && m.is_active && m.employee_type === 'technician') {
          allMembers.push({ phone: m.phone, name: m.name || m.username || 'unknown' });
        }
      }
    }
    ...
    console.log(`[discoverCrewPhone] picked ${picked.name} (${picked.phone}) — ${testMember ? 'TEST account' : 'first available'}`);
