...
async function osirisLogin(page, baseUrl, credentials, role = 'owner') {
  ...
  try {
    ...
    // Phase B: Wait for the credentials screen. Some portals render text
    // inputs without type=password (React Native Web w/ secureTextEntry), so
    // we wait for ANY input to appear, then resolve the password field via
    // attribute sniffing in findPasswordField().
    ...
    // change OR for the Sign In button to disappear.
    const advanced = await Promise.race([
      page.waitForFunction(
        (startUrl) => location.href !== startUrl && !/\/(login|signin|sign-in|auth)(\/|$|\?)/i.test(location.href),
        baseUrl,
        { timeout: 12000 }
