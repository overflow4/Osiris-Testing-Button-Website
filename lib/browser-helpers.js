...
// and :has-text only walks <button> elements.
const ROLE_PATTERNS = {
  owner: /\b(operator|owner|founder|admin)\b/i,
  technician: /\b(technician|employee|cleaner|crew|staff)\b/i,
};
...
async function osirisLogin(page, baseUrl, credentials, role = 'owner') {
  ...
  // it'll drop straight into the dashboard. Log out first so we land back on
  // the role-selection screen and can pick the requested role.
  try {
  ...
  try {
    // Phase A: Role selection. The login screen renders role buttons (Operator/Owner,
    // Employee/Technician, etc). We pick by the requested role's regex.
    const clickedText = await clickRoleButton(page, role);
    console.log(`${tag} role button clicked: ${clickedText || 'NONE FOUND'}`);
    if (!clickedText) {
      // Dump the visible button text so we can see what's actually on screen
      ...
      console.log(`${tag} no role match. Visible clickables: ${JSON.stringify(visibleButtons)}`);
      return false;
    }

    // Phase B: Wait for the credentials screen to render. If the password
    // field doesn't appear, role selection didn't actually take us forward.
    const passField = await page.waitForSelector('input[type="password"]', { timeout: 8000 }).catch(() => null);
    if (!passField) {
      console.log(`${tag} password field never appeared after clicking "${clickedText}"`);
      return false;
    }
    ...
    // that appears BEFORE the "[ INITIALIZE ]" button in DOM order.
    ...
    // Phase D: Verify login actually completed by waiting for the password
    // field to disappear. URL-based checks were unreliable — clicking a role
    // button can change the URL without ending the auth flow.
    const passGone = await page.waitForSelector('input[type="password"]', { state: 'detached', timeout: 12000 })
      .then(() => true).catch(() => false);
    if (!passGone) {
      console.log(`${tag} password field still present after submit — login rejected`);
      return false;
    }
    await page.waitForTimeout(2500);
  } catch (e) {
    console.log(`${tag} unexpected error: ${e.message}`);
    return false;
  }

  // Final sanity: not parked on an auth/role URL
  const url = page.url();
